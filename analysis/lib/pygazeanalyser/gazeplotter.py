...
def draw_fixations(fixations, answer_loc, move_color, dispsize, imagefile=None, durationsize=True, durationcolour=True, alpha=0.5, savefilename=None):
	...
	# convert old x position to new x position
	if answer_loc != None:
		new_x_locs = [100, 175, 250, 325, 400, 475, 550, 625, 700]
		new_y_locs = [413, 338, 263, 188]
		old_x_locs = [-305, -229, -153, -77, -1, 75, 151, 227, 303]
		old_y_locs = [113, 37, -39, -115]
		answer_x = new_x_locs[numpy.where(old_x_locs == answer_loc[0])[0][0]]
		answer_y = new_y_locs[numpy.where(old_y_locs == answer_loc[1])[0][0]]
		ax.plot(answer_x,
				answer_y, 'o',
				color=move_color)
...
def animate_raw(x, y, answer_loc, move_color, dispsize, imagefile=None, savefilename=None):
	# image
	fig, ax = draw_display(dispsize, imagefile=imagefile)

	# Calculate the desired downsampling factor
	...
	# Create an initial scatter plot with a single dot
	dot, = ax.plot(x[0], y[0], 'o', color=COLS['aluminium'][0], markeredgecolor=COLS['aluminium'][5])
	# invert the y axis, as (0,0) is top left on a display
	ax.invert_yaxis()
	...
	def update(frame):
		# Update the position of the dot for each frame
	...
	# total_time = len(x) / 1000 # this should be in real time
	#
	...
	#
	# new_interval = 1000 / fps


	# Create the animation with frame skipping
...
def draw_display(dispsize, imagefile=None):
	...
	screen = numpy.zeros((dispsize[1],dispsize[0],3), dtype='float32')#dtype='uint8')
	...
	if imagefile != None:
		...
		# JN added on 12/14: Resize the board image to 700 x 350
		original_pil_image = Image.fromarray((img * 255).astype('uint8'))
		resized_pil_image = original_pil_image.resize((700, 350))
		img = numpy.array(resized_pil_image) / 255.0
		img = img[:, :, :3]
